...
impl Stacks {
    pub fn new(input : BufReader<File>) -> Self {
        ...
        for line in input.lines() {
            ...
            let mut c = -1;
            line.retain(|_| {c+=1; return c % 4 == 1 });
            tmp.push(line);
        }
    ...
    for (c,line) in reader.lines().enumerate() {
        ...
        if line.len() > 4 {
            if &line[0..4] == "move" { 
                ...
                stacks.do_instruct(v[3].parse().unwrap() , v[5].parse().unwrap(), v[1].parse().unwrap());
            }
        }
...
pub fn day5_2() {
    ...
    for (c,line) in reader.lines().enumerate() {
        ...
        if line.len() > 4 {
            if &line[0..4] == "move" { 
                let v: Vec<&str>= line.split(|c| c == ' ').collect();
                stacks.do_instruct_9001(v[3].parse().unwrap() , v[5].parse().unwrap(), v[1].parse().unwrap());
            }
        }
